...
description: Check whether a change to an API contract would break its consumers, using brake, before the change is written. Use when editing, generating or reviewing an OpenAPI/Swagger document, a .proto file, or a GraphQL SDL schema — removing or renaming a field, changing a type, adding a required parameter, changing a status code, tightening security — or when asked "is this change backward compatible?", "will this break clients?", or "can I remove this field?".
...
## 5. Know the limits before you rely on the answer
