...
M0–M6 are done, and so are M7 (protobuf), M8 (GraphQL) and AsyncAPI 2.x/3.x.
All four ingesters produce the same `Contract` and share one comparator;
`check`, `analyze`, `diff`, `explain` and `consumers` work; text, JSON, SARIF,
GitHub workflow commands and GitLab Code Quality all render.

A contract may span several files: a cross-file `$ref` is resolved through a
`DocumentResolver`, hermetically, within the document's own directory.
OpenAPI 3.1 type arrays, `prefixItems` tuples and `discriminator` mappings are
modelled and compared — through the existing rules, not new ones.
...
- **`AGENTS.md` and `.junie/skills/` are generated copies** of `CLAUDE.md` and
  `.claude/skills/`. Edit the canonical file, then `make agents`.
  `tests/agent_guidance_is_consistent.rs` fails if they drift, because no agent
  tool follows a pointer between the three places reliably.
- **The base contract and the head contract get *different* resolvers.** A
  cross-file `$ref` on the baseline side must be read out of the baseline's own
  revision — `ResolvedBaseline::origin` says which — while the head side reads
  the working tree. Sharing one resolver put today's shared schema on both
  sides, so a field deleted from a file two documents `$ref` was missing from
  base and head alike and the run reported clean.
  `check::tests::a_break_in_a_shared_document_is_not_hidden_by_the_working_tree`
  is the guard.
- **A resolver path is relative to the *document*, never to `source`.** For a
  baseline read out of git, `source` is a label like `rev:HEAD` and names no
  directory at all. Each resolver is rooted at the contract's own directory,
  which is also the boundary a `$ref` cannot reach past.
- **`TypeRef` is public API that forge matches on.** Adding a variant, or a
  field to one, breaks every exhaustive `match` and struct-variant literal
  downstream. It is sometimes the right thing to do — say so in the release
  notes rather than discovering it in forge's build.
- **[design/06-architectural-evolution.md](design/06-architectural-evolution.md)
  is a roadmap, not a description.** Some of it is built and some is not:
  `proto-field-unreserved` and `proto-enum-value-unreserved` are proposals, and
  reusing a reserved number or name is currently reported as `contract-partial`
  via an unmodelled entry. Never document a proposal as behaviour — check
  `src/rules/catalogue.rs`, which is the only list of rules that exist.
...
src/contract/resolver.rs   Hermetic multi-document $ref resolution
...
src/baseline.rs    file / git / git-merge-base, via gix; and the baseline's resolver
src/render/        text, json, sarif, github, gitlab
...
AGENTS.md          Generated from CLAUDE.md by `make agents`
.junie/skills/     Generated from .claude/skills/ by `make agents`
```
...
make agents         # regenerate AGENTS.md and .junie/skills from CLAUDE.md
